...
void main() {
  test("get volunteerings", () async {
    ...
    final volunteeringsToInsert =
        MockVolunteeringList().volunteering[0].toJsonReduced();
    final volunteeringsArray = [];
    volunteeringsArray.add(volunteeringsToInsert);
    final firebaseData = {'values': volunteeringsArray};
    final documentReference =
        fakeFirestore.collection('ser_manos_data').doc('voluntariados');
    await documentReference.set(firebaseData);

    // Pass mocked firebase to api
    final firebaseCloudstore =
        FirebaseCloudstoreITBA.withFirestore(fakeFirestore);

    // Retrieve from firebase
    final volunteeringList =
        VolunteeringList.withFirebaseCloudstore(firebaseCloudstore);
